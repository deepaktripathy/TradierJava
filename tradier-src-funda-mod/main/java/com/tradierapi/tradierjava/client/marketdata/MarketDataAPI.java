...
import com.tradierapi.tradierjava.client.marketdata.request.Interval;
import com.tradierapi.tradierjava.client.marketdata.request.MinuteInterval;
...
public class MarketDataAPI {
   ...
   public List<HistoricPrice> getPriceHistory(@Nonnull String symbol, @Nullable Interval interval,
      ...
      try {
         ...
         httpBuilder.addQueryParameter("interval", (interval == null) ? Interval.daily.toString() : interval.toString());
         httpBuilder.addQueryParameter("session_filter", (interval == null) ? SessionFilter.all.toString() : sessionFilter.toString());
         if (fromDate != null) {
   ...
   public TimeSalesResponse getTimeSales(@Nonnull String symbol, @Nullable MinuteInterval interval,
         @Nullable LocalDate fromDate, @Nullable LocalDate toDate, @Nullable SessionFilter sessionFilter) {
      Objects.requireNonNull(symbol);
      ...
      try {
         ...
         HttpUrl.Builder httpBuilder = HttpUrl.parse(url).newBuilder();
         httpBuilder.addQueryParameter("symbol", symbol);
         httpBuilder.addQueryParameter("interval", (interval == null) ? MinuteInterval.tick.toString() : interval.toString());
         httpBuilder.addQueryParameter("session_filter", (sessionFilter == null) ? SessionFilter.all.toString() : sessionFilter.toString());
         if (fromDate != null) {
            httpBuilder.addQueryParameter("start", isoDate(fromDate));
         }
         if (toDate != null) {
