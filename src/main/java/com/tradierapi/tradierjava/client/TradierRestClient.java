...
public class TradierRestClient implements TradierClient {
		...
		try {
			...
			if (isPost) {
				RequestBody body = new FormBody.Builder()
						.add("symbols", symbols.stream().collect(Collectors.joining(",")))
		...
		try {
			...
			if (responseCode == 200) {
				...
				if (optionNode.isObject()) {
					...
					List<Option> myQuotes = mapper.treeToValue(jsonTree.findPath("options"), Options.class)
							.getOptions();
					if (myQuotes != null && !myQuotes.isEmpty())
	...
	public Map<String, String> getOptionExchangeCodeMap() {
		String exchMapStr = "A=NYSE Amex Options" + ",B=BOX Options Exchange"
				+ ",C=Chicago Board Options Exchange (CBOE)" + ",H=ISE Gemini"
				+ ",I=International Securities Exchange (ISE)" + ",M=MIAX Options Exchange" + ",N=NYSE Arca Options"
				+ ",O=Options Price Reporting Authority (OPRA)" + ",P=MIAX PEARL" + ",Q=NASDAQ Options Market"
				+ ",T=NASDAQ OMX BX" + ",W=C2 Options Exchange" + ",X=NASDAQ OMX PHLX" + ",Z=BATS Options Market";
	...
	public Map<String, String> getStockExchangeCodeMap() {
		String exchMapStr = "A=NYSE MKT" + ",B=NASDAQ OMX BX" + ",C=National Stock Exchange" + ",D=FINRA ADF"
				...
				+ ",L=Long Term Stock Exchange" + ",M=Chicago Stock Exchange" + ",N=NYSE" + ",P=NYSE Arca"
				+ ",Q=NASDAQ OMX" + ",S=NASDAQ Small Cap" + ",T=NASDAQ Int" + ",U=OTCBB" + ",V=OTC other" + ",W=CBOE"
				+ ",X=NASDAQ OMX PSX" + ",G=GLOBEX" + ",Y=BATS Y-Exchange" + ",Z=BATS";
