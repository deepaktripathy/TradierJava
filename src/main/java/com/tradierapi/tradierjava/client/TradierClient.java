...
/**
 * Currently this works and returns data for a single account. Decide if the
 * client should lock to the initially supplied account that can be set via
 * constructor or be flexible to take an accountId argument for most of the
 * functions, like getOrders() or getPositions().
 * 
...
public interface TradierClient {
	...
	/**
	 * Returns quote as a GET response.
	...
	/**
	 * Returns quotes as a GET response.
	 * Includes greeks for options only
	 */
	List<Quote> getQuotes(@Nonnull List<String> symbols, @Nullable Boolean includeGreeks);

	/**
	 * Returns quotes as a POST response. 
	 * Includes greeks for options only
	 */
	List<Quote> postQuotes(@Nonnull List<String> symbols, @Nullable Boolean includeGreeks);
	...
	/**
	 * Returns all the daily extended session history as available between two
	 * dates
	 */
	List<HistoricPrice> getPriceHistory(@Nonnull String symbol, @Nullable LocalDate fromDate,
	...
	/**
	 * Search for companies by name. May returns multiple.
	 * 
	 * incluseIndexes will include indexes in search query
	 */
	List<Security> searchCompany(@Nonnull String name, @Nullable Boolean includeIndexes);
	 ...
	 * suggested to use the overloaded method with a supplied type.
	 */
	Optional<Security> lookupSymbol(@Nonnull String symbol);

	/**
	 * Look up stocks, etfs, indices as well as individual option symbols based on
	 * the supplied type.
	 * 
	 * May returns multiple results.
	 * 
	 * Note: This will be extremely slow since this will do a huge lookup. It is
	 * suggested to use the overloaded method with a supplied type.
	 */
	List<Security> lookupSymbol(@Nonnull String symbol, @Nonnull List<ExchangeCode> exchangeCodes,
			@Nonnull List<SecurityType> types);
	 ...
	 */
	List<Order> getOrders(@Nullable Integer page, @Nullable Boolean includeTags);
	...
	/**
	 * Returns all types of history for an account TODO: Implement it (not enabled
	 * for sandbox, need to test in live)
	 */
//    List<Position> getAccountHistory();

	/**
	 * Get cost basis information for a specific user account. This includes
	 * information for all closed positions.
	 */
	...
	/**
	 * Returns these parameters: account_type, total_cash, cash_available,
	 * stock_buying_power, option_buying_power
	 */
	...
	/**
	 * An option chain is all calls+puts for a single expiration. Since this may
	 * take a lot of resources, have another method to filter out these.
	 */
	...
	/**
	 * Returns available strike prices for a option
	 */
	List<Double> getOptionStrikes(OptionStrikePricesRequest strikePricesRequest);

	/**
	 * Tradier returns single digit exchange codes in some of the responses. This
	 * returns a static map of all the names to the respective exchange codes.
	 */
	Map<String, String> getStockExchangeCodeMap();

	/**
	 * Tradier returns single digit exchange codes in some of the responses. This
	 * returns a static map of all the names to the respective option exchange
	 * codes.
	 */
